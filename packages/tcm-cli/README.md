[![NPM version](https://img.shields.io/npm/v/@aw-web-design/tcm-cli.svg?style=flat)](https://npmjs.org/package/@aw-web-design/tcm-cli)
[![Blazing Fast](https://badgen.now.sh/badge/speed/blazing%20%F0%9F%94%A5/green)](https://npm.im/@aw-web-design/tcm-cli) [![Blazing Fast](https://badgen.now.sh/badge/speed/blazing%20%F0%9F%94%A5/green)](https://npm.im/@aw-web-design/tcm-cli) [![Blazing Fast](https://badgen.now.sh/badge/speed/blazing%20%F0%9F%94%A5/green)](https://npm.im/@aw-web-design/tcm-cli)
[![Conventional Commits](https://img.shields.io/badge/Conventional%20Commits-1.0.0-yellow.svg)](https://conventionalcommits.org)

> a fork of the official tsdx.

```bash
$ npm install tcm-cli -D   # for npm users
$ yarn add tcm-cli -D      # for yarn users
$ pnpm install tcm-cli -D  # for pnpm users
```
...
TCM comes with the "battery-pack included" and is part of a complete TypeScript breakfast:
...
- Jest test runner setup with sensible defaults via `tcm test`
- ESLint with Prettier setup with sensible defaults via `tcm lint`
- Zero-config, single dependency
- Escape hatches for customization via `.babelrc.js`, `jest.config.js`, `.eslintrc.js`, and `tcm.config.js`/`tcm.config.ts`

## Quick Start

```bash
npx tcm-cli create mylib
cd mylib
yarn start
```
...
Runs the project in development/watch mode. Your project will be rebuilt upon changes. TCM has a special logger for your convenience. Error messages are pretty printed and formatted for compatibility VS Code's Problems tab.
...
Aside from just bundling your module into different formats, TCM comes with some optimizations for your convenience. They yield objectively better code and smaller bundle sizes.

After TCM compiles your code with TypeScript, it processes your code with 3 Babel plugins:
...
`tcm build` will output an ES module file and 3 CommonJS files (dev, prod, and an entry file). If you want to specify a UMD build, you can do that as well. For brevity, let's examine the CommonJS output (comments added for emphasis):
...
AS you can see, TCM stripped out the development check from the production code. **This allows you to safely add development-only behavior (like more useful error messages) without any production bundle size impact.**
...
TCM's rollup config [removes getters and setters on objects](https://github.com/palmerhq/tsdx/blob/1f6a1b6819bb17678aa417f0df5349bec12f59ac/src/createRollupConfig.ts#L73) so that property access has no side effects. Don't do it.

#### Advanced `babel-plugin-dev-expressions`

TCM will use `babel-plugin-dev-expressions` to make the following replacements _before_ treeshaking.
...
Note: TCM doesn't supply an `invariant` function for you, you need to import one yourself. We recommend https://github.com/alexreardon/tiny-invariant.
...
Note: TCM doesn't supply a `warning` function for you, you need to import one yourself. We recommend https://github.com/alexreardon/tiny-warning.

### Using lodash

If you want to use a lodash function in your package, TCM will help you do it the _right_ way so that your library does not get fat shamed on Twitter. However, before you continue, seriously consider rolling whatever function you are about to use on your own. Anyways, here is how to do it right.
...
Import your lodash method however you want, TCM will optimize it like so.
...
TCM will rewrite your `import kebabCase from 'lodash/kebabCase'` to `import o from 'lodash-es/kebabCase'`. This allows your library to be treeshakable to end consumers while allowing to you to use `@types/lodash` for free.

> Note: TCM will also transform destructured imports. For example, `import { kebabCase } from 'lodash'` would have also been transformed to `import o from "lodash-es/kebabCase".
...
TCM can automatically rollup TypeScript type definitions into a single `index.d.ts` file via [rollup-plugin-tcm](https://github.com/Swatinem/rollup-plugin-tcm) plugin. To enable types rollup, add `--rollupTypes` flag to your `build` and `watch` scripts.

```json
    "build": "tcm build --rollupTypes",
    "start": "tcm watch --rollupTypes",
```

[rollup-plugin-tcm](https://github.com/Swatinem/rollup-plugin-tcm) was seen to cause issues when using json and less imports. Use with caution.
...
> **❗⚠️❗ Warning**: <br>
> These modifications will override the default behavior and configuration of TCM. As such they can invalidate internal guarantees and assumptions. These types of changes can break internal behavior and can be very fragile against updates. Use with discretion!

TCM uses Rollup under the hood. The defaults are solid for most packages (Formik uses the defaults!). However, if you do wish to alter the rollup configuration, you can do so by creating a file called `tcm.config.js` (or `tcm.config.ts`) at the root of your project like so:
...
```typescript
import { TcmOptions, RollupOptions } from 'tcm-cli';
...
You can add your own `.babelrc` to the root of your project and TCM will **merge** it with [its own Babel transforms](./src/babelPluginTsdx.ts) (which are mostly for optimization), putting any new presets and plugins at the end of its list.

### Jest

You can add your own `jest.config.js` to the root of your project and TCM will **shallow merge** it with [its own Jest config](./src/createJestConfig.ts).

### ESLint

You can add your own `.eslintrc.js` to the root of your project and TCM will **deep merge** it with [its own ESLint config](./src/createEslintConfig.ts).
...
TCM was originally ripped out of [Formik's](https://github.com/jaredpalmer/formik) build tooling.
TCM has several similarities to [@developit/microbundle](https://github.com/developit/microbundle), but that is because Formik's Rollup configuration and Microbundle's internals had converged around similar plugins.

### Comparison with Microbundle

Some key differences include:

- TCM includes out-of-the-box test running via Jest
- TCM includes out-of-the-box linting and formatting via ESLint and Prettier
- TCM includes a bootstrap command with a few package templates
- TCM allows for some lightweight customization
- TCM is TypeScript focused, but also supports plain JavaScript
- TCM outputs distinct development and production builds (like React does) for CJS and UMD builds. This means you can include rich error messages and other dev-friendly goodies without sacrificing final bundle size.
...
This runs Jest, forwarding all CLI flags to it. See [https://jestjs.io](https://jestjs.io) for options. For example, if you would like to run in watch mode, you can run `tcm test --watch`. So you could set up your `package.json` `scripts` like:
...
Examples
  ...
  $ tcm lint src test --max-warnings 10
